...
public class SnakePlaneController : CollidableObject
{
    ...
    private bool _hitVerticalBound;
    ...
    void Move()
    {
        // changes speed smoothly
        verticalSpeed = Mathf.Lerp(verticalSpeed, goalSpeed, Time.deltaTime * 2);
        // changes vertical direction when Lerp is completed
        if (Mathf.Abs(verticalSpeed - goalSpeed) <= 0.001)
        {
            ChangeDirection();
        }
    ...
    // Changes the plane's vertical direction.
    ...
    void CheckBounds()
    {
        // Changes Direction once if the snake plane hits a vertical boundary
        if (transform.position.y > boundary.Top)
        {
            transform.position = new Vector3(transform.position.x, boundary.Top, 0);
            if(!_hitVerticalBound)
            {
                _hitVerticalBound = true;
                ChangeDirection();
                verticalSpeed = 0;
            }
        }
        ...
        {
            transform.position = new Vector3(transform.position.x, boundary.Bottom, 0);
            if (!_hitVerticalBound)
            {
                _hitVerticalBound = true;
                ChangeDirection();
